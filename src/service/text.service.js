...
const wordCount = async (fileId) => {
  const data = fileId
    ? fs.readFileSync(`${defaultTextFilePath}${fileId}`, "utf-8")
    : fs.readFileSync(`${defaultTextFilePath}sample.txt`, "utf-8");
...
const largestWordsinParagraphs = async (fileId) => {
  ...
  for (let i = 0; i < sanitizedData.length; i++) {
    ...
    for (let j = 0; j < sanitizedData[i].length; j++) {
      if (sanitizedData[i][j].length > l) {
        l = sanitizedData[i][j].length;
      }
    }
    largestWords.push(l);
  }
